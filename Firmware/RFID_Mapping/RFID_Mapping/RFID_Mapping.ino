...
byte room3009[]= {226,0,0,23,2,5,0,97,23,0,105,134};   // Room 14

byte roomHOME[]= {226,0,0,23,2,5,0,96,23,0,105,125};   // Room 15

/*************************************
* RFID Flags 
*************************************/
byte atHome = TRUE;   // Home flag
byte atRoom = FALSE;  // Room flag
...
void loop() 
{
  ...
  while(rx_msg[0] == 0x00)  // Wait for room number input
  {
  ...
  checkRoomLocation(desiredRoom);   // Check direction of room from base & send info to main arduino

  // Clear zero turn bits for when desired room reached
  bitClear(tx_msg[1], ZL_BIT);
  bitClear(tx_msg[1], ZR_BIT);
  
  temp_msg = tx_msg[1];

  if(bitRead(temp_msg, SIDE_BIT) == RIGHT)
    roomSide = RIGHT;
  else
    roomSide = LEFT;
  ...
  while(atHome == FALSE)
  {
    ...
    {
      digitalWrite(LED1, HIGH);  // Turn LED on to signify room reached
      
      atRoom = TRUE;

      if(desiredRoom != homeRoom)
      {
        bitClear(temp_msg, MTR_BIT);  // Set motors off
        bitSet(temp_msg, ROOM_BIT);   // Desired room reached
  
        if(roomSide == LEFT)
          bitSet(temp_msg, ZL_BIT);
        else
          bitSet(temp_msg, ZR_BIT);
  
        tx_msg[1] = temp_msg;
        Serial1.write(tx_msg, 2);  // RFID BOARD TO MAIN ARDUINO
        ...
        while(atRoom == TRUE)
        {
          ...
          if(rx_msg[0] == homeRoom)
            desiredRoom = homeRoom;
  
          tx_msg[0] = desiredRoom;
  
          bitSet(temp_msg, MTR_BIT);  // Turn motors on
          bitClear(temp_msg, ROOM_BIT);
      ...
      else  // desiredRoom == homeRoom
      {
        bitClear(temp_msg, MTR_BIT);    // Set motors off
        bitSet(temp_msg, HOME_BIT);     // Home reached
        bitClear(temp_msg, ROOM_BIT);   // Desired room reached
...
void checkRoomLocation(byte des_room) {  
  if(des_room == 0x02 || des_room == 0x04 || des_room == 0x06 || des_room == 0x08 || des_room == 0x0A || des_room == 0x0C)
  {
    ...
    bitSet(tx_msg[1], ZL_BIT);      // Set zero turn left 
  ...
  else
  {
    ...
    if(des_room == 0x01 || des_room == 0x03 || des_room == 0x05 || des_room == 0x07 || des_room == 0x09)
    {
      tx_msg[0] = desiredRoom;
      ...
      bitClear(tx_msg[1], HOME_BIT);  // Leaving home
      bitClear(tx_msg[1], SIDE_BIT);  // Room on left side of hallway
      bitSet(tx_msg[1], ZR_BIT);      // Set zero turn right
...
void checkCurrentRoom() {
  if(myEPC[7] == roomHOME[7] && myEPC[11] == roomHOME[11])
    ...
    currentRoom = 0x08;
  else if(myEPC[7] == room3009[7] && myEPC[11] == room3009[11])
    currentRoom = 0x09;
